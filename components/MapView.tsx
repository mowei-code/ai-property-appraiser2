...
export const MapView: React.FC<MapViewProps> = ({ property, properties, filters, onSelectProperty, onMapMarkerSelect, onLocationSelect, onClose }) => {
  ...
  const onMapMarkerSelectRef = useRef(onMapMarkerSelect);
  useEffect(() => {
    onMapMarkerSelectRef.current = onMapMarkerSelect;
  }, [onMapMarkerSelect]);
  ...
  useEffect(() => {
    ...
    if (mainMarkerRef.current) {
      if ((mainMarkerRef.current as any)?._clickCleanup) {
        (mainMarkerRef.current as any)._clickCleanup();
      }
    ...
    if (property && property.latitude && property.longitude) {
      ...
      const handlePositionUpdate = async (lat: number, lng: number, markerInstance: any) => {
        const loadingProperty: Property = {
          ...(property || mockProperties[0]),
          id: `loading_update_${Date.now()}`,
        ...
        onMapMarkerSelectRef.current(loadingProperty);
        if (markerInstance) {
          markerInstance.setTooltipContent(tRef.current('mapView_queryingAddress')).openTooltip();
        }

        try {
          const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lng}&accept-language=zh-TW`);
          if (!response.ok) throw new Error('Reverse geocoding failed');
          const data = await response.json();

          let fetchedAddress = formatNominatimAddress(data);
          if (!fetchedAddress) {
            fetchedAddress = `Lat: ${lat.toFixed(5)}, Lon: ${lng.toFixed(5)}`;
          }
          ...
          const fallbackProperty: Property = {
            ...loadingProperty,
            id: `error_update_${Date.now()}`,
            address: tRef.current('mapView_addressQueryFailed'),
            district: tRef.current('mapView_unknownArea'),
          };
          onMapMarkerSelectRef.current(fallbackProperty);
          if (markerInstance) {
            markerInstance.setTooltipContent(tRef.current('mapView_addressQueryFailed')).openTooltip();
          }
        }
      };

      newMainMarker.on('dragend', (event: any) => {
        const marker = event.target;
        const position = marker.getLatLng();
        handlePositionUpdate(position.lat, position.lng, marker);
      });

      // Add click handler to map to move marker
      const onMapClick = (e: any) => {
        // Prevent conflicting with existing markers if they stop propagation correctly, 
        // but Leaflet map click often fires even if marker is clicked unless handled.
        // In this logic, we just move the main marker.
        handlePositionUpdate(e.latlng.lat, e.latlng.lng, newMainMarker);
      };
      map.on('click', onMapClick);

      // Store cleanup function for this render cycle
