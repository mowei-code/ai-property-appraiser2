      ...
      <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 sm:p-8 border border-orange-400 dark:border-orange-500">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
              {settings.language === 'zh-TW' ? (
                <>
                  請輸入地址或地圖查詢後
                  <button onClick={() => props.onOpenSettings('preferences')} className="text-blue-600 hover:underline mx-1 font-medium inline-block">
                    (設定 Gemini API Key)
                  </button>
                  點擊「AI智慧為您估價參考」按鈕開始。
                </>
          ...
          <button
            ...
            className="group relative flex items-center justify-center gap-3 px-6 py-3 bg-white dark:bg-slate-800 border-2 border-emerald-500 text-emerald-600 dark:text-emerald-400 font-bold text-lg rounded-2xl shadow-[0_0_15px_rgba(16,185,129,0.2)] hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:-translate-y-0.5 transition-all duration-300 overflow-hidden w-full sm:w-auto"
          >
      ...
      {props.comparisonList.length > 0 && (
        <div className="fixed bottom-6 left-0 right-0 flex justify-center z-30 pointer-events-none">
          <div className="pointer-events-auto bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl p-3 pr-4 rounded-full shadow-2xl border border-orange-300 dark:border-orange-700 flex items-center gap-4 animate-fade-in-up">
            ...
            <button
              onClick={props.comparisonList.length < 2 ? () => props.onOpenSettings('upgrade') : props.onCompareClick}
