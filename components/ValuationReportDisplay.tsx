...
export const ValuationReportDisplay: React.FC<ValuationReportDisplayProps> = ({
    ...
    const [activeTab, setActiveTab] = useState('overview'); // 'overview' | 'details' | 'market' | 'scenario'
    ...
    const handleExportComprehensive = (format: ExportFormat) => {
        // ... (Keep existing export logic, it is long but I must copy it or it will be lost if I replace the whole body. 
        // Wait, I am replacing from line 326. I MUST include the export logic!)
        if (!valuation) return;
    ...
    const renderHero = () => (
        <div className="relative overflow-hidden bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 mb-8">
    ...
    const renderAnalysis = () => (
        <div id="comprehensive-analysis-section" className="mb-6">
            <div className="flex justify-between items-center mb-4">
    ...
    const tabs = [
        { id: 'overview', label: t('valuationReportFor') || 'Overview', icon: <BuildingLibraryIcon className="h-5 w-5" /> },
        { id: 'details', label: t('aiComprehensiveAnalysis') || 'Details', icon: <DocumentTextIcon className="h-5 w-5" /> },
        { id: 'market', label: t('marketSummary') || 'Market', icon: <ChatBubbleLeftRightIcon className="h-5 w-5" /> },
        { id: 'scenario', label: t('aiScenarioAnalysis') || 'Scenario', icon: <SparklesIcon className="h-5 w-5" /> },
    ];

    return (
        <div className="mt-8 animate-fade-in pb-20 md:pb-0">
            {renderHeader()}
            ...
            <div className="md:hidden sticky top-[72px] z-30 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm -mx-4 px-4 py-2 border-b border-slate-200 dark:border-slate-800 mb-6 flex overflow-x-auto gap-4 no-scrollbar">
                {tabs.map(tab => (
                    <button
                        ...
                        className={`flex flex-col items-center gap-1 min-w-[64px] pb-2 transition-all ${activeTab === tab.id ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}
                    >
                        <div className={`p-2 rounded-xl ${activeTab === tab.id ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                            {tab.icon}
                        </div>
                        <span className="text-xs font-medium whitespace-nowrap">{tab.label.length > 6 ? tab.label.substring(0, 6) + '..' : tab.label}</span>
                    </button>
                ))}
            </div>
            ...
            <div className="md:hidden">
                {activeTab === 'overview' && (
                    <div className="animate-fade-in">
                        {renderHero()}
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
                            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4">{t('marketSummary')}</h3>
                            <p className="text-slate-700 dark:text-slate-300 leading-loose">{valuation.marketSummary}</p>
                        </div>
                    </div>
                )}
                {activeTab === 'details' && (
                    <div id="comprehensive-analysis-section" className="animate-fade-in mb-6">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                                <span className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-xl text-blue-600">
                                    <SparklesIcon className="h-6 w-6" />
                        ...
                        {/* Hidden on mobile view, but visible when printed to ensure full report context */}
                        <div className="hidden print:block bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 mb-6">
                            <p className="text-slate-700 dark:text-slate-300 leading-loose text-lg">{valuation.marketSummary}</p>
                        </div>
                ...
                {activeTab === 'market' && <div className="animate-fade-in">{renderMarket()}</div>}
                {activeTab === 'scenario' && <div className="animate-fade-in">{renderScenario()}</div>}
            </div>
